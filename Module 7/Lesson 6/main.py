...
def get_essay_details():
    ...
    essay_type = input("What is the type of essay you are writing? ").strip()
    lengths = ["300 words", "600 words", "900 words", "1200 words", "2000 words"]
...
def feedback_and_refinement():
    ...
    if rating != 5:
        feedback = input("Provide feedback (tone, structure, ettc.): ").strip()
...
def run_activity():
    ...
    if not details["topic"] or not details["essay_type"]:
        print("Please provide a topic and an essay type to continue.")
        return
