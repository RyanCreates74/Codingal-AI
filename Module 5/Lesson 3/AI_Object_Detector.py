...
def font(sz=18):
    for f in ("DejaVeSans.tf","arial.ttf"):
...
def ask_image():
    ...
    while True:
        ...
        if os.path.getsize(p)/(1024**2) > MAX_MB: print("⚠️ Too big (>8MB)."); continue
...
def infer(path, img_bytes, tries=8):
    ...
    for _ in range(tries):
        ...
        if r.status_code == 200:
            d = r.json()
            if isinstance(d, list) and "error" in d:
...
def main():
    ...
    if counts:
        ...
        print("\n⚠️ Disclaimer: This is an AI model demo. "
              ...
              "Use it for fun and learning, not for safety-crictical decisions.")
